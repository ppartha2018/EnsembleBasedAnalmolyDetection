...
'''
baseline knn-15 classifier applied as individual method
...
clf = KNN(k=25)
knn_dist = clf.predict(data_X)
knn20_auc_score = roc_auc_score(data_y, knn_dist)
    
print "\nKnn baseline method:"
print "Knn-base Classifier AUC Score (Avg) :", knn20_auc_score
...
print "\nKnn Ensembles on whole dataset:"
print "Knn Ensemble method with k=15,20 and 25:"
print "Knn-Ensemble Classifier AUC Score: (Avg)", np.mean(clf_auc_scores)
#print "Knn-Ensemble Classifier AUC Score: (Std)", np.std(clf_auc_scores)

'''
knn ensemble method2 -1
Sampling - split dataset into 50 near equal sub datasets
Run a designated knn classifier with k=3 on each subsample
concatenate results
...
for i in range(0,25):
    ...
    clf_auc_scores.append(clf_ensemle_auc_score)
print "\nKnn Ensembles on (50) subsamples dataset with k=3:"
print "Knn-Ensemble Classifier AUC Score: (Avg)", np.mean(clf_auc_scores)
print "Knn-Ensemble Classifier AUC Score: (std)", np.std(clf_auc_scores)

'''
knn ensemble method2 -2
Sampling - split dataset into 10 near equal sub datasets
Run a designated knn classifier with k=5 on each subsample
concatenate results
compare with original class labels to get the roc_auc score
'''
...
for i in range(0,25):
    ...
    clf_ensemle_auc_score =  roc_auc_score(data_samples_y, clf_ensemble_scores)
    clf_auc_scores.append(clf_ensemle_auc_score)
print "\nKnn Ensembles on (10) subsamples dataset with k=3:"
print "Knn-Ensemble Classifier AUC Score: (Avg)", np.mean(clf_auc_scores)
print "Knn-Ensemble Classifier AUC Score: (Std)", np.std(clf_auc_scores)
...
for j in range(20,50):
    ...
    clf_auc_scores.append(clf_ensemle_auc_score)
print "\nKnn Ensembles on (50) subsamples dataset with k=3,4,5 on each subsample:"
print "Knn-Ensemble Classifier AUC Score: (Avg)", np.mean(clf_auc_scores)
print "Knn-Ensemble Classifier AUC Score: (Std)", np.std(clf_auc_scores)
...
print "\nKnn Ensembles on (50 subsamples - outliers equally distributed) subsamples dataset with k=3,5,10 on each subsample:"
print "Knn-Ensemble Classifier AUC Score:", lof_ensemble_auc_score